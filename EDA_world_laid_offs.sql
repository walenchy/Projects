...
--  fetch out max_total_laid_off, max_plo & min_plo
SELECT MAX(total_laid_off), MAX(percentage_laid_off), MIN(percentage_laid_off)
FROM layoffs_staging2;

-- see the percentage_laid_off is 100% wrt highest funds raise 
SELECT *
FROM layoffs_staging2
...
;
-- i want to know the sum of total laid off by each company
SELECT company, SUM(total_laid_off)
FROM layoffs_staging2
GROUP BY company
ORDER BY 2 DESC;
-- check out for max and min date
SELECT MIN(`date`), MAX(`date`)
FROM layoffs_staging2
;

-- sum of total_laid off per industry
SELECT industry, SUM(total_laid_off)
FROM layoffs_staging2
GROUP BY industry
ORDER BY 2 DESC;

-- sum of total_laid off per country
SELECT country, SUM(total_laid_off)
FROM layoffs_staging2
GROUP BY country
ORDER BY 2 DESC;

-- work on max funds raised per country
...
-- max/avg fund along with country and industry
...
-- check the date along with sum of total laid off
...
-- Use substring to extract month from the date column
...
-- USE CTE for company_year_ranking
